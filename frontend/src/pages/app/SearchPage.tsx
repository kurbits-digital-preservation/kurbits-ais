      ...
      <div className={styles.resultBody}>
        ...
        <div className={styles.resultMeta}>
          ...
          {(item.date_start || item.date_end) && (
            <>
              <span className={styles.dot}>·</span>
              <span>{item.date_start}{item.date_end && item.date_end !== item.date_start
                ? `–${item.date_end}` : ''}</span>
            </>
          )}
        </div>
...
function FilterPanel({
  params, setParam, clearParam, levels,
}: {
      ...
      <div className={styles.filterGroup}>
        ...
        {[
          ...
          <label key={opt.value} className={styles.radioLabel}>
            <input type="radio" checked={types === opt.value}
              onChange={() => setParam('types', opt.value)} />
            {opt.label}
          </label>
        ))}
      </div>
          ...
          <div className={styles.filterGroup}>
            ...
            <div className={styles.dateRow}>
              <div className="form-group" style={{ margin: 0 }}>
                <label style={{ fontSize: 'var(--text-xs)', color: 'var(--color-ink-faint)' }}>From</label>
              ...
              <div className="form-group" style={{ margin: 0 }}>
                <label style={{ fontSize: 'var(--text-xs)', color: 'var(--color-ink-faint)' }}>To</label>
  ...
  return (
    <div className={styles.pagination}>
      ...
      <div className={styles.paginationBtns}>
        <button className="btn btn-ghost btn-sm btn-icon" disabled={page <= 1}
          onClick={() => onPage(page - 1)}>
          <ChevronLeft size={16} />
        </button>
        {Array.from({ length: Math.min(pages, 7) }, (_, i) => {
          const p = pages <= 7 ? i + 1 : page <= 4 ? i + 1
            : page >= pages - 3 ? pages - 6 + i : page - 3 + i
          return (
            <button key={p}
        ...
        <button className="btn btn-ghost btn-sm btn-icon" disabled={page >= pages}
          onClick={() => onPage(page + 1)}>
...
export default function SearchPage() {
  ...
  // Load all hierarchy levels for filter dropdown
  ...
  const { data: allLevels } = useQuery({
    queryKey: ['all-hierarchy-levels', hierarchyTypes?.map((h: any) => h.id)],
    queryFn: async () => {
      if (!hierarchyTypes?.length) return []
      const results = await Promise.all(
        hierarchyTypes.map((h: any) =>
          hierarchyApi.listLevels(h.id).then(r => r.data.data as any[])
        )
      )
      return [...new Set(results.flat().map((l: any) => l.name))].sort()
    },
  ...
  const hasFilters = ['status', 'level', 'date_from', 'date_to', 'agent_type']
    ...
    <div className={styles.page}>
      ...
      <div className={styles.searchBar}>
        ...
        >
          ...
          Filters
          {hasFilters && <span className={styles.filterCount}>
      ...
      <div className={styles.body}>
        ...
        <div className={styles.results}>
            ...
            <div className={styles.statsBar}>
              {isFetching ? (
                ...
                <div className={styles.statsLeft}>
                  ...
                  {data.node_total > 0 && (
                    <span className={styles.typeStat}>{data.node_total} resource{data.node_total !== 1 ? 's' : ''}</span>
                  )}
                  {data.agent_total > 0 && (
                    <span className={styles.typeStat}>{data.agent_total} agent{data.agent_total !== 1 ? 's' : ''}</span>
                  )}
                </div>
              ) : null}
            </div>
          ...
          {!q && (
            <div className={styles.emptyState}>
              ...
              <p className={styles.emptyHint}>
                Searches across titles, descriptions, scope notes, and custom metadata fields
              </p>
            </div>
          )}
          ...
          {q.length === 1 && (
            <div className={styles.emptyState}>
              <p className={styles.emptyHint}>Type at least 2 characters</p>
            </div>
          )}

          {!isLoading && q.length >= 2 && total === 0 && (
            <div className={styles.emptyState}>
              <Search size={32} style={{ opacity: 0.15, marginBottom: 'var(--space-3)' }} />
