...
function SchemeEditor({ initial, onSave, onCancel, saving }: {
  ...
  const handleSave = () => {
    ...
    try {
      ...
      setJsonError('Headers is not valid JSON.'); return
    }
    try {
      if (form.generator_body_text.trim())
        body = JSON.parse(form.generator_body_text)
    } catch {
      setJsonError('Request body template is not valid JSON.'); return
    }
      ...
      <div className={styles.editorGrid}>
        <div className="form-group">
          ...
          <input value={form.name} onChange={e => set('name', e.target.value)}
            placeholder="e.g. ARK, DOI, Handle" autoFocus />
        </div>
        <div className="form-group">
        ...
        <div className="form-group" style={{ gridColumn: '1 / -1' }}>
          <label><Link2 size={12} /> Resolver URL template</label>
          <input value={form.url_template} onChange={e => set('url_template', e.target.value)}
            placeholder="https://n2t.net/{value}" />
          <span className="form-hint">
            {'{value}'} is replaced with the identifier to build a clickable link.
          </span>
        </div>
        <div className="form-group" style={{ gridColumn: '1 / -1' }}>
          <label><Zap size={12} /> Generator service URL</label>
          ...
          <span className="form-hint">
            Optional. Setting this enables a "Generate" button on records.
          </span>
        </div>

        {form.generator_url.trim() && (<>
          <div className="form-group">
            <label>Shoulder / namespace</label>
            <input value={form.generator_shoulder} onChange={e => set('generator_shoulder', e.target.value)}
              placeholder="e.g. prod-dataset" />
            <span className="form-hint">Substituted as {'{shoulder}'} in the body.</span>
          </div>
          <div className="form-group">
            ...
            <span className="form-hint">Where the minted ID sits in the response. Blank = "value".</span>
          </div>
          <div className="form-group" style={{ gridColumn: '1 / -1' }}>
            ...
            <span className="form-hint">
              Built per record and sent as {'{target_url}'}. {'{ref_code}'} and {'{node_id}'} are substituted.
            </span>
          </div>
          <div className="form-group" style={{ gridColumn: '1 / -1' }}>
          ...
          <div className="form-group" style={{ gridColumn: '1 / -1' }}>
            ...
            <span className="form-hint">
              Placeholders: {'{node_id}'} {'{ref_code}'} {'{title}'} {'{target_url}'} {'{shoulder}'}.
            </span>
          </div>
        </>)}
...
export default function IdentifierSchemesTab() {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) => alert(err?.response?.data?.message ?? 'Could not create'),
  })
  const updateMutation = useMutation({
    mutationFn: ({ id, form }: { id: number; form: SchemeForm }) =>
      identifierSchemesApi.update(id, toPayload(form)),
    onSuccess: () => { invalidate(); setEditingId(null) },
    onError: (err: any) => alert(err?.response?.data?.message ?? 'Could not save'),
  })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: invalidate,
    onError: (err: any) => alert(err?.response?.data?.message ?? 'Could not delete'),
  })
  ...
  return (
    <div className={styles.tab}>
      <div className={styles.header}>
        <div>
          <h3 className={styles.heading}><Fingerprint size={16} /> Identifier schemes</h3>
          <p className={styles.sub}>
            Persistent and external identifier types (ARK, Handle, DOI…) that records can carry.
          </p>
        </div>
        {!creating && (
          <button className="btn btn-primary btn-sm" onClick={() => setCreating(true)}>
            <Plus size={13} /> New scheme
          </button>
        )}
      </div>
      ...
      <div className={styles.list}>
        {(schemes ?? []).length === 0 && !creating && (
          <p className={styles.empty}>No identifier schemes yet.</p>
        )}
        {(schemes ?? []).map((s: any) => (
          editingId === s.id ? (
            <SchemeEditor
            ...
            <div key={s.id} className={`${styles.row} ${!s.is_active ? styles.rowInactive : ''}`}>
              <div className={styles.rowMain}>
                <div className={styles.rowTop}>
                  ...
                  {s.has_generator && <span className={styles.tag}><Zap size={9} /> Generator</span>}
                  {s.url_template && <span className={styles.tag}><Link2 size={9} /> Resolver</span>}
                  {!s.is_active && <span className={styles.inactiveTag}>Inactive</span>}
                </div>
                {s.description && <span className={styles.rowDesc}>{s.description}</span>}
              </div>
              <div className={styles.rowActions}>
                <label className={styles.toggle} title={s.is_active ? 'Active' : 'Inactive'}>
                  <input type="checkbox" checked={s.is_active}
                    onChange={e => toggleMutation.mutate({ id: s.id, is_active: e.target.checked })} />
                </label>
                <button className="btn btn-ghost btn-sm btn-icon"
                  onClick={() => setEditingId(s.id)} title="Edit">
                  <Pencil size={13} />
                </button>
                <button className="btn btn-ghost btn-sm btn-icon"
                  onClick={() => { if (confirm(`Delete scheme "${s.name}"?`)) deleteMutation.mutate(s.id) }}
                  title="Delete">
                  <Trash2 size={13} />
                </button>
              </div>
